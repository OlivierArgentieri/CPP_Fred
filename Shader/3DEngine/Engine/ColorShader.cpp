...
bool ColorShader::Initialize(ID3D11Device *_device, HWND _hWnd)
{
	...
	result = InitializeShader(_device, _hWnd, "color.vs", "color.ps");
	if(!result)
	{
		return false;
	}
...
bool ColorShader::InitializeShader(ID3D11Device *_device, HWND _hWnd, char *_vsFilename, char *_psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(fullVSName, NULL, NULL, "ColorVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
	if(FAILED(result))
	{
	...
	result = D3DX11CompileFromFile(fullPSName, NULL, NULL, "ColorPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if(errorMessage)
