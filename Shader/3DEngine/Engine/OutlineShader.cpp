...
OutlineShader::OutlineShader(const OutlineShader& other)
{
}
...
bool OutlineShader::Initialize(ID3D11Device *_device, HWND _hWnd)
{
	bool result;

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(_device, _hWnd, "outline.vs", "outline.ps"); // Color 
...
bool OutlineShader::InitializeShader(ID3D11Device *_device, HWND _hWnd, char *_vsFilename, char *_psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(fullVSName, NULL, NULL, "OutlineShaderVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
	if(FAILED(result))
	{
	...
	result = D3DX11CompileFromFile(fullPSName, NULL, NULL, "OutlineShaderPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if(errorMessage)
...
void OutlineShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char* shaderFilename)
{
	char* compileErrors;
	ofstream fout;
...
bool OutlineShader::SetShaderParameters(
		...
		ID3D11ShaderResourceView* _texture)
{
	...
	worldInverse = DirectX::XMMatrixTranspose(worldInverse);
...
bool OutlineShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, DirectX::XMMATRIX worldMatrix, DirectX::XMMATRIX viewMatrix,
...
void OutlineShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
